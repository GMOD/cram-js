# ADR 0004 — Size the slice cache above one query, and reclaim it when idle

Status: Accepted (`cacheSize` 20,000 → 1,000,000 records; adds
`cacheIdleTimeoutMs`, default 3 minutes, and `clearFeatureCache`)

## Context

`featureCache` is bounded by decoded **record count**, defaulting to 20,000. The
number predated any measurement of what one query needs.
...
anyway. Working set for a single query, measured with an unbounded cache on the
jb2bench CRAMs:
...
Every short-read width was over the line. Six-window 50kb pan, second pass:
...
The old default also did not bound what it claimed to. `evictionPolicy: 'batch'`
(ADR 0003) spares everything the batch touched, so a 20,000-record budget was
...
- **A record count cannot bound memory, and this does not pretend to.** There is
  ...
  (ADR 0003). One useful consequence: the budget only ever binds on short-read
  data, where records are small and numerous. Long-read slices are few and huge
  — 2,991 records for a 50kb window at 1000x — so they never approach it.
...
- **The idle timeout is what makes the number affordable.** `cacheSize` is
  ...
  of the track. Without it a parked tab holds its whole last view until the
  track closes, times every open track. Timed from the last _read_ of a slice,
  not the decode, so panning back and forth over one region never expires it.

- **`'batch'` still stands (ADR 0003), but for the reason given there and not as
  a substitute for a big enough budget.** @gmod/bam measured `'batch'` failing
  to rescue an undersized budget — identical refill count to `'lru'`, and 1.7x
  worse on memory (@gmod/bam ADR 0013). It wins on a _repeated identical_ query,
  which is what ADR 0003 measured; it does not make a too-small budget work.
...
Working sets measured with `cacheSize: Infinity` — with a budget applied,
...
note). Pan is six 50kb windows stepping 25kb and doubling back, timed on the
