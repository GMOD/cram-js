// Define WebAssembly module interface
...
class XzContext {
  ...
  inEnd: number
  outStart: number
  mem8!: Uint8Array // Using definite assignment assertion
  mem32!: Uint32Array // Using definite assignment assertion
  outPos: number = 0 // Initialize with default value

  constructor(moduleInstance: WasmModuleInstance) {
    this.exports = moduleInstance.exports
    this.memory = this.exports.memory
    this.ptr = this.exports.create_context()

    // Initialize memory views first
    this.mem8 = new Uint8Array(this.memory.buffer, this.ptr)
    this.mem32 = new Uint32Array(this.memory.buffer, this.ptr)

    // Now it's safe to access mem32 properties
    this.bufSize = this.mem32[0] || 0
    this.inStart = (this.mem32[1] || 0) - this.ptr
    this.inEnd = this.inStart + this.bufSize
    this.outStart = (this.mem32[4] || 0) - this.ptr
  }

  supplyInput(sourceDataUint8Array: Uint8Array): void {
    this._refresh()
  ...
  getNextOutput(): OutputResult {
    ...
    // Make sure mem32[5] exists before using it
    const outPos = this.mem32[5] || 0
  ...
  needsMoreInput(): boolean {
    ...
    const inPos = this.mem32[2] || 0
    const inSize = this.mem32[3] || 0
    return inPos === inSize
  }

  outputBufferIsFull(): boolean {
    const outPos = this.mem32[5] || 0
    return outPos === this.bufSize
  }
  ...
  _refresh(): void {
    // Initialize or refresh the memory views
    this.mem8 = new Uint8Array(this.memory.buffer, this.ptr)
    this.mem32 = new Uint32Array(this.memory.buffer, this.ptr)
  }
}
...
export class XzReadableStream extends ReadableStream<Uint8Array> {
  static _moduleInstancePromise: Promise<void>
  static _moduleInstance: WasmModuleInstance

  static async _getModuleInstance(): Promise<void> {
    ...
    // Cast to unknown first to avoid type compatibility issues
    XzReadableStream._moduleInstance =
      module.instance as unknown as WasmModuleInstance
  }

  constructor(compressedStream: ReadableStream<Uint8Array>) {
    let xzContext: XzContext
